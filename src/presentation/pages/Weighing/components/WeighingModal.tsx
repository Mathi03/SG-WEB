...
const WeighingModal = ({
  ...
  const form = useForm<WeighingSchema>({
    ...
    defaultValues: {
      ...
      weighedAt: data?.weighedAt ? new Date(data.weighedAt) : new Date(),
  ...
  const { run: runCreate, loading: loadingCreate } = useRequest(
    (weighing) => WeighingController.create(weighing),
    {
      manual: true,
      onSuccess: () => {
        toast.success("Pesaje creado exitosamente.");
        onAction();
        onClose(false);
      },
      onError: (error) => {
  ...
  const { run: runUpdate, loading: loadingUpdate } = useRequest(
    (weighing) => WeighingController.update(data?.id ?? 0, weighing),
    {
      manual: true,
      onSuccess: () => {
        toast.success("Pesaje actualizado exitosamente.");
        onAction();
        onClose(false);
      },
      onError: (error) => {
  ...
  const onSubmit = (data: WeighingSchema) => {
    if (action === "add") {
      runCreate({
        ...data,
        weighedAt: format(data.weighedAt, "yyyy-MM-dd"),
      });
    } else {
      const sendData = {
        weighedAt: format(data.weighedAt, "yyyy-MM-dd"),
  ...
  return (
    <Modal
      width="800px"
      height="auto"
      title={`${action === "add" ? "Agregar" : "Editar"} Pesaje`}
      onClose={(e: boolean) => {
        onClose(e);
      }}
    >
