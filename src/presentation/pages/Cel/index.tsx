...
import CelModal from "./components/CelModal";
import { useRequest } from "ahooks";
import CelController from "@infrastructure/controllers/CelController";
import { format } from "date-fns";
import { CelSchema } from "@infrastructure/validations/celValidation";
...
const Cel = () => {
  ...
  const columns: ColDef[] = [
    ...
    },
    {
      ...
      filterValueGetter: ({ data }) => {
        return new Date(format(new Date(data.dateObserved), "MM/dd/yyyy"));
      },
      filterParams: {
        buttons: ["apply", "clear"],
      },
    ...
    },
    {
      ...
      pinned: "right",
      width: 120,
      cellRenderer: ({ data }: { data: CelSchema }) => {
        return (
  ...
  const {
    data: listCel,
    loading: loadingListCel,
    refresh: refreshListCel,
  } = useRequest(() => CelController.search({ farmId: currentFarm?.id ?? 0 }), {
